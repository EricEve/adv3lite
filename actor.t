...
//            local sp;
    ...
    /* 
     *   If this is set to true then the execute() method will look in methods
     *   as well as strings to try to see what objects need to be included in
     *   the object table. This is more comprehensive, but it is also riskier,
     *   since executing code out of context at preinit may cause run-time
...
/* 
 *   We use a transient object to capture the output of a topicResponse method
 *   since we need to be able to undo any side-effects that running it might
 ...
 *   the conversationManager objNameTable if they want to use the more
 *   economical option but it's not picking up all the objects that need to be
 *   stored.
 *
 *   To use this fallback, modify extraObjPreinit or otherwise set its
 *   extraObjList property to a list of *strings* holding keys to the objects to
